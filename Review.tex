...
\fig{gru}における$z$と$r$がゲートの役割でゲート$z$で新しく値を更新するかどうかを決定し，$r$で前の情報を無視するか利用するかを判断する．
...
\section{教師なし学習}
機械学習の手法には，\ref{sec:NeuralNetwork}〜\ref{sec:3Danalysis}節で説明したように，ラベルの貼られているデータセットを用いて学習する教師あり学習と，その反対に，データセットはあっても，そのデータセットの特性を示したラベルが与えられていない場合のデータセットを用いて学習する教師なし学習がある．本節では教師なし学習について述べる．


\subsection{Autoencoder}
教師なし学習で画像の特徴を抽出する方法としてオートエンコーダ(Autoencoder)がある\cite{hinton2006reducing}．画像の場合におけるオートエンコーダの手法とは，ある画像から情報を圧縮する「エンコーダ(encoder)」と言われる部分と，その圧縮した情報から画像を復元する「デコーダ(decoder)」の2つからなる．入力とデコーダから復元された画像が同じ画像になるようにニューラルネットワークで学習させる．この学習の結果，潜在変数は似てる画像どうしで近い値になるように変化し，この分布を見れば教師ラベルがなくても画像の分類を行うことができる．

\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\linewidth]{fig/chapter2/auto_encoder}
	\caption{AutoEncoder Network\cite{hinton2006reducing}}
	\label{fig:autoencoder}
\end{figure}


\subsection{Variational Autoencoder}
Variational Autoencoder(VAE)はオートエンコーダの派生である\cite{kingma2013auto}．VAEは，オートエンコーダの「エンコーダ」と「デコーダ」は同じネットワーク構造であるが，\eq{vae}のようにデータセットの潜在変数の分布が正規分布になるような制約を加えて学習を行う点でオートエンコーダと異なる．ここで，$N$は正規分布，$\mu$は平均，$\sigma$は分散を示す．
...
敵対的生成ネットワーク(Generative Advarsarial Network: GAN)は2014年に提案された手法である\cite{GAN}．\fig {GAN}のようにGANではGeneratorとDiscriminatorの2つのネットワークが存在する．Generatorは訓練データと同じような画像を生成するネットワークで，Discriminatorは入力されたデータが訓練データから来たものかGeneratorで生成されたものかを識別するように学習する．
...
推論モデルは\fig{m2_discriminative}であり，以下のように定義される．
...
生成モデルは\fig{m2_generative}であり，以下のように定義される．
...
機械学習による病理診断アルゴリズムを評価するために，3つの指標がある．AccuracyとSpecificty(特異度)とSensitivity(感度)である．なぜ，Accuracyだけではなく，SensitivityとSpecificityが必要かを説明する図が\fig{confusion_ex}である．この場合だとAccuracyが8割弱で良いと判断することもできるが，Sensitivityに注目すると1割しかないので，この機械学習のモデルでは，正常の画像を正常であると認識することができていないことが分かる．また見落とし防止を目標にする場合は，S
