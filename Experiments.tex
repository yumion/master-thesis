...
\begin{table}[H]
	...
	\begin{tabular}{cc}\toprule
		...
		Emission wavelength (nm) & 440.0--420.0  \\ 
		Excitation wavelength (nm) & 405.0  \\ 
		Pinhole size ($\upmu$m) & 20.00  \\ 
		Channel modality & Laser Scan Confocal, PMT  \\ \bottomrule
	\end{tabular}
\end{table}
...
\subsection{テスト画像}
入力画像の中心を$130\times 130$ pxでクロッピングし，これをテストデータとした．\fig {クロップ}に各検体のクロッピング画像を示す．
...
\begin{table}[H]
	...
	\begin{tabular}{cc}\toprule
		...
		Resolution & $20\times (0.62005 \upmu \mathrm{m / pxel})$ \\ 
...
まず，両データの解像度を揃え，サンプル間で画像を正規化をした．事前学習の画像が，本検体の画像よりも解像度が4倍あった．事前学習のデータセットは画像サイズが異なるデータも多少あったため，画像サイズが$755 \times 522$ pxである画像151枚を利用し，その画像を$432 \times 299$ pxにリサイズした．また，本研究の検体をウィンドウサイズが130 pxとなるようにスライドウィンドウで画像を取得した．その際，背景の割合が0.1\%以上あるものは取り除いた．130 pxで切り抜いた画像を299 pxにリサイズして入力画像とした．
学習に用いる際には，事前学習の画像では$432 \times 299$ pxから$256 \times 256$ pxに切り抜き，本研究の検体では$299 \times 299$ pxをランダムに$256 \times 256$ pxに切り抜いて学習の教師データとした．
...
URL : \url{https://github.com/aiorhiroki/researches}
