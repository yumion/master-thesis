...
\end{figure}
事前学習で用意する画像と本研究で使用する検体が類似している必要があるため，前処理を行った．解像度を揃えて，サンプル間で正規化をした．事前学習の画像が，本検体の画像よりも解像度が4倍あった．そこで画像サイズが(755pix, 522pix)である枚数が151枚を利用し，その画像を(432, 299)にリサイズした．本研究の検体をウィンドウサイズが130pixとなるようにスライドウィンドウで画像を取得した．その際，背景の割合が0.1\%以上あるものは取り除いた．130pixで切り抜いた画像を299pixにリサイズして入力画像とした．また学習に用いる際には，事前学習の画像では(432, 299)から(256, 256)に切り抜き，本研究の検体では(299, 299)を(256, 256)に切り抜いて学習の教師データとした．

\subsection{バッチ学習}
本研究は，3次元画像を解析するため，学習に利用する画像サイズやニューラルネットワークのパラメータ数が大きくなるため11GBのGPUにはメモリに乗らないことがある．そこで，画像をバッチに分けてバッチごとに画像を読み込みGPUで計算を行った．
...
\subsection*{2次元画像}
用いたネットワーク構造は，InceptionV3, Xception, Inception\_Resnetの４つである
